...
z_loc = 82;
brain_slice = brain_rescaled(:,:,z_loc)';
sliceMASK = surf(repmat(84, size(brain_slice)), maskslice);
...
for i = 1:921
    StartTime = round(last_time*(find(Order==i)/921))+1;
    R(i,StartTime:StartTime+N2target-1) = r;
    R(i,StartTime+N2target:end) = 1;
    Endtime(i) = StartTime+N2target-1;
end
U = unique(Endtime);
for i = 1:length(U)
    thratEnd(i) = min(pc_thal_colors_ind(find(Endtime==U(i))))+256;
end
    
Ind_thr = [ones(1,9)*513 thratEnd];
seeds2plot_colorind = pc_thal_colors_ind(seeds2plot_ind)+256;
for j = 1:60
    for i = 1:921
    y_coords = [pc_patch_coords_start(i) pc_patch_coords_start(i) pc_patch_coords_start(i+1) pc_patch_coords_start(i+1)]+5; 
    v = [x_coords(1) y_coords(1) z_coords(1);x_coords(2) y_coords(2) z_coords(2);x_coords(3) y_coords(3) z_coords(3);x_coords(4) y_coords(4) z_coords(4)];
    ...
    v3 = find_point_on_line(v,v2,R(i,j));
    
    pc_patches(i).Vertices = v3;
    
    if seeds2plot_ind(i)==0
        pc_patches(i).FaceAlpha=1-R(i,j);
    end
    
    if seeds2plot_ind(i)==1 && R(i,j) == 1
    ...
    for i = 1:length(seeds2plot) 
        if seeds2plot_colorind(i) >= Ind_thr(j)
        sSeeds(i).FaceAlpha = 0;
        end
    end
    
    newslicevals_temp = brain_slice;
    newslicevals_temp(newslicevals>=Ind_thr(j)) = newslicevals(newslicevals>=Ind_thr(j));
    %newslicevals_temp(newslicevals<=256) = newslicevals(newslicevals<=256);
    slicesurf.CData = newslicevals_temp;
