function flatMapPlots = plotData2MouseFlatmap(data,cmap,offset)
...
for i = 1:38
    ...
    flatMapPlots.plot(i) = fill(Mouseflatmap_boundary_offset{IND}(:,1),Mouseflatmap_boundary_offset{IND}(:,2),cort_pc_color(i,:),'EdgeColor',[0 0 0]);
    hold on
end
...
for i = 1:length(NaN_regions)
    IND = NaN_regions(i);
    flatMapPlots.plot(i+38) = fill(Mouseflatmap_boundary_offset{IND}(:,1),Mouseflatmap_boundary_offset{IND}(:,2),[.5 .5 .5],'EdgeColor',[0 0 0]);
    hold on
end
