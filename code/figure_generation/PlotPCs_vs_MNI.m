...
for XYZ = 1:3
    ...
    FigCaption{XYZ} = [annotlabels{XYZ},', PC',num2str(grad),' score with ',lower(SPATIAL_DIR{XYZ}),' axis (MNI ',SPATIAL_AXIS{XYZ},'-coordinate; Pearson''s r(',num2str(length(pc_thal)-2),') = ',num2str(R),', p = ',num2str(pval),', CI = [',num2str(CIL(1,2)),', ',num2str(CIU(1,2)),'], two-tailed).'];
             
    disp(FigCaption{XYZ})
    
    SpearmanResult = ['PC',num2str(grad),' score with ',lower(SPATIAL_DIR{XYZ}),' axis (MNI ',SPATIAL_AXIS{XYZ},'-coordinate): Spearmans''s rs(',num2str(length(pc_thal)-2),') = ',num2str(RHO),', p = ',num2str(RHOpval),', two-tailed.'];
    disp(SpearmanResult)
