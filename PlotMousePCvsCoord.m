...
iter = 1; 
    ...
    for XYZ = 1:3
        ...
        figure('Position',[162   233   713   592])
            ...
            FigCaption{XYZ} = [annotlabels{XYZ},', mPC',num2str(grad),' score with ',lower(SPATIAL_DIR{XYZ}),' axis (CCFv3 ',SPATIAL_AXIS{XYZ},'-coordinate): Pearson''s r(',num2str(length(mpc_thal)-1),') = ',num2str(RHO),', p = ',num2str(pval),', CI = [',num2str(CIL(1,2)),', ',num2str(CIU(1,2)),'], two-tailed.'];
